...
const copy: Record<string, Copy> = {
  en: { title: 'Movies & Series', sub: 'Movies, series, cartoons, posters, genres and trailers', movie: 'Movies', series: 'Series', cartoon: 'Cartoons', genre: 'Genre', all: 'All', search: 'Search title...', trailer: 'Trailer/Search', watch: 'Official page', rating: 'Rating', noData: 'No titles found', top: 'Top Trending', anticipated: 'Most Anticipated', yearFrom: 'From year', yearTo: 'To year', allYears: 'All years', separate: 'Separated by type and genre', openPage: 'More details', details: 'More details' },
  az: { title: 'Kino və Seriallar', sub: 'Kino, serial, multfilm, şəkil, janr və trailer', movie: 'Kino', series: 'Serial', cartoon: 'Multfilm', genre: 'Janr', all: 'Hamısı', search: 'Ad ilə axtar...', trailer: 'Trailer/Axtar', watch: 'Rəsmi səhifə', rating: 'Reytinq', noData: 'Nəticə tapılmadı', top: 'Top Trend', anticipated: 'Ən çox gözlənilənlər', yearFrom: 'İldən', yearTo: 'İlə qədər', allYears: 'Bütün illər', separate: 'Növ və janra görə ayrılıb', openPage: 'Daha ətraflı', details: 'Daha ətraflı' },
  ru: { title: 'Кино и сериалы', sub: 'Фильмы, сериалы, мультфильмы, постеры, жанры и трейлеры', movie: 'Фильмы', series: 'Сериалы', cartoon: 'Мультфильмы', genre: 'Жанр', all: 'Все', search: 'Поиск по названию...', trailer: 'Трейлер/поиск', watch: 'Официальная страница', rating: 'Рейтинг', noData: 'Ничего не найдено', top: 'Топ трендов', anticipated: 'Самые ожидаемые', yearFrom: 'С года', yearTo: 'До года', allYears: 'Все годы', separate: 'Разделено по типу и жанру', openPage: 'Подробнее', details: 'Подробнее' },
  de: { title: 'Filme & Serien', sub: 'Filme, Serien, Cartoons, Poster, Genres und Trailer', movie: 'Filme', series: 'Serien', cartoon: 'Cartoons', genre: 'Genre', all: 'Alle', search: 'Titel suchen...', trailer: 'Trailer/Suche', watch: 'Offizielle Seite', rating: 'Bewertung', noData: 'Keine Titel gefunden', top: 'Top-Trends', anticipated: 'Meist erwartet', yearFrom: 'Ab Jahr', yearTo: 'Bis Jahr', allYears: 'Alle Jahre', separate: 'Nach Typ und Genre getrennt', openPage: 'Mehr Details', details: 'Mehr Details' },
  tr: { title: 'Film ve Diziler', sub: 'Filmler, diziler, çizgi film, posterler, türler ve fragmanlar', movie: 'Filmler', series: 'Diziler', cartoon: 'Çizgi film', genre: 'Tür', all: 'Hepsi', search: 'Başlık ara...', trailer: 'Fragman/Ara', watch: 'Resmi sayfa', rating: 'Puan', noData: 'Sonuç bulunamadı', top: 'Top Trend', anticipated: 'En çok beklenenler', yearFrom: 'Yıldan', yearTo: 'Yıla kadar', allYears: 'Tüm yıllar', separate: 'Türe ve kategoriye göre ayrıldı', openPage: 'Daha detaylı', details: 'Daha detaylı' },
}
...
export default function EntertainmentWidget() {
  ...
  const api = useMemo(() => { const params = new URLSearchParams({ type: mode, lang }); if (yearFrom) params.set('yearFrom', yearFrom); if (yearTo) params.set('yearTo', yearTo); if (genre) params.set('genre', genre); if (query.trim()) params.set('q', query.trim()); return `/api/entertainment?${params.toString()}` }, [genre, lang, mode, query, yearFrom, yearTo])
  const trendingApi = useMemo(() => { const params = new URLSearchParams({ type: mode, mode: 'trending', lang }); if (yearFrom) params.set('yearFrom', yearFrom); if (yearTo) params.set('yearTo', yearTo); return `/api/entertainment?${params.toString()}` }, [lang, mode, yearFrom, yearTo])
  const anticipatedApi = useMemo(() => `/api/entertainment?type=${mode}&mode=anticipated&lang=${lang}`, [lang, mode])
  ...
  const items = (Array.isArray(data) ? data : []).filter(item => item.type === mode)
  const trending = (Array.isArray(trendingData) ? trendingData : []).filter(item => item.type === mode).slice(0, 4)
  const anticipated = (Array.isArray(anticipatedData) ? anticipatedData : []).filter(item => item.type === mode).slice(0, 4)
