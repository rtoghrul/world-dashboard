...
export default function EntertainmentPage() {
  return (
    <Suspense fallback={<EntertainmentFallback />}>
      <EntertainmentPageContent />
    </Suspense>
  )
}

function EntertainmentFallback() {
  return (
    <div className="min-h-screen bg-gray-950">
      <Header />
      <main className="max-w-screen-2xl mx-auto px-4 py-6">
        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-3">
          {Array.from({ length: 12 }).map((_, i) => <div key={i} className="animate-pulse"><div className="aspect-[2/3] rounded-xl bg-gray-800 mb-2" /><div className="h-3 bg-gray-800 rounded" /></div>)}
        </div>
      </main>
    </div>
  )
...
function EntertainmentPageContent() {
  ...
  const api = useMemo(() => {
    ...
    if (sp.get('yearFrom')) params.set('yearFrom', sp.get('yearFrom') || '')
    if (sp.get('yearTo')) params.set('yearTo', sp.get('yearTo') || '')
