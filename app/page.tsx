...
export default function HomePage() {
  ...
  const travelLabel = lang === 'az' ? 'Səyahət Qiymət Axtarışı' : lang === 'ru' ? 'Поиск цен на поездки' : lang === 'de' ? 'Reisepreis-Suche' : lang === 'tr' ? 'Seyahat Fiyat Arama' : 'Travel Price Search'
  ...
  const sections = useMemo(() => [
    ...
    { id: 'travel', label: travelLabel, keywords: ['flight', 'flights', 'ucus', 'uçuş', 'travel', 'hotel', 'check24', 'booking', 'skyscanner', 'kayak'] },
  ...
  const visibleSections = useMemo(() => {
    ...
    if (normalizedSearch) base = base.filter(section => [section.label, ...section.keywords].join(' ').toLowerCase().includes(normalizedSearch))
  ...
  const togglePinned = (id: string) => { const pinnedSet = new Set(sectionPrefs.pinned); if (pinnedSet.has(id)) pinnedSet.delete(id); else pinnedSet.add(id); savePrefs({ pinned: Array.from(pinnedSet), hidden: sectionPrefs.hidden }) }
  const toggleHidden = (id: string) => { const hiddenSet = new Set(sectionPrefs.hidden); if (hiddenSet.has(id)) hiddenSet.delete(id); else hiddenSet.add(id); savePrefs({ pinned: sectionPrefs.pinned.filter(p => p !== id), hidden: Array.from(hiddenSet) }) }
  ...
  useEffect(() => {
    ...
    sections.map(s => s.id).forEach(id => { const el = document.getElementById(id); if (!el) return; const observer = new IntersectionObserver(([entry]) => { if (entry.isIntersecting) setActiveSection(id) }, { rootMargin: '-40% 0px -55% 0px', threshold: 0 }); observer.observe(el); observers.push(observer) })
  ...
  return (
    <div className="min-h-screen bg-gray-950">
      ...
      <main className="max-w-screen-2xl mx-auto px-4 py-6 space-y-6" key={refreshKey}>
        ...
        {(visibleIds.has('news') || visibleIds.has('travel')) && <section id="news" className="grid grid-cols-1 lg:grid-cols-2 gap-6">{visibleIds.has('news') && <NewsWidget />}{visibleIds.has('travel') && <div id="travel"><TravelComparisonWidget /></div>}</section>}
