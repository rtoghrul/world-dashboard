...
import { ArrowLeft, ExternalLink, PiggyBank, Utensils, Pill, Shirt, ShoppingBag, Car, Baby, Home, BadgeEuro } from 'lucide-react'
...
const groups = [
  {
    title: 'Food savings & food sharing',
    subtitle: 'Discounted restaurant, cafe and surplus food in Germany',
    icon: Utensils,
    items: [
  ...
  {
    title: 'Cheap pharmacies & medicine price comparison',
    subtitle: 'Useful before buying OTC medicine, vitamins and family products',
    icon: Pill,
    items: [
      ...
      { name: 'ApocliaCounter', url: 'https://www.apolista.de', note: 'Compare pharmacy prices before buying' },
  ...
  {
    title: 'Cheap clothes, furniture & home items',
    subtitle: 'Second-hand, nearly free items and family-friendly shopping',
    icon: Shirt,
    items: [
  ...
  {
    title: 'Cashback, price comparison & coupons',
    subtitle: 'Check these before buying anything online',
    icon: BadgeEuro,
    items: [
  ...
  {
    title: 'Family, groceries & everyday savings',
    subtitle: 'Very useful for families living in Germany',
    icon: Baby,
    items: [
  ...
  {
    title: 'Car, fuel & travel savings',
    subtitle: 'Useful for your RAV4, family trips and parking',
    icon: Car,
    items: [
    ...
    <div className="min-h-screen relative z-[1] bg-gray-950">
      ...
      <main className="max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <div className="flex items-center gap-3 mb-6">
          <Link href="/" className="w-10 h-10 rounded-xl bg-white/[0.04] border border-white/[0.06] flex items-center justify-center hover:bg-white/[0.08] transition-all">
          ...
          <div>
            <h1 className="text-2xl font-bold gradient-text-static">Germany Savings Apps</h1>
                  ...
                  {group.items.map(item => (
                    <a key={item.name} href={item.url} target="_blank" rel="noreferrer" className="group rounded-xl border border-white/[0.06] bg-white/[0.02] hover:bg-white/[0.04] hover:border-emerald-500/25 p-3 transition-all">
                      <div className="flex items-center justify-between gap-2">
                        <h3 className="text-white text-xs font-semibold">{item.name}</h3>
